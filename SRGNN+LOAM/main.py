...
def main():
    ...
    train_data = Data(train_data, opt, ht_dict, device, opt.max_len, mode='train', shuffle=True)
    test_data = Data(test_data, opt, ht_dict, device, train_data.len_max, mode='test', shuffle=False)
    
    model = SRGNN(opt, n_items, device).to(device)
    ...
    for epoch in range(opt.epoch):
        print('-' * 100)
        print('Epoch: ', epoch)
        _, results = train_test(model, train_data, test_data, n_items, ht_dict, wandb)
