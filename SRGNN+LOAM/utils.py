...
def get_metric_scores(scores, targets, k, ht_dict, ht_idxs, eval):
    ...
    # head acc and cov
    head_idxs = np.setdiff1d(np.arange(len(targets)), tail_idxs)
    ...
    eval[9] += np.unique(sub_scores[score_tail_idxs[0], score_tail_idxs[1]]).tolist()
    return eval


def data_masks(all_usr_pois, item_tail, mode, max_len=200):
    if mode == 'train':
    ...
    for upois in all_usr_pois:
        ...
        le = len(new_upois)
        us_pois.append(new_upois + item_tail * (len_max - le))
...
def NWA(batch_graph, targets, max_len, total_n_walks, alpha=0.3, walk_len=10, n_walks=5, rand=random.Random(220)):
    walks = []
    aug_targets = []
    ## code for random walk alpha true 
    for tar in targets:
...
class Data():
    def __init__(self, data, opt, ht_dict, device, max_len, mode, shuffle=False):
        inputs = data[0]
        inputs, len_max = data_masks(inputs, [0], mode, max_len)
    ...
    def get_slice(self, i):
        ...
        items = torch.LongTensor(np.array(items))
        A =  torch.FloatTensor(np.array(A))
        alias_inputs = torch.LongTensor(np.array(alias_inputs))
        targets = torch.LongTensor(np.array(targets)-1)
        
        return items, A, alias_inputs, targets, [head_idxs, tail_idxs], org_inputs_len
...
class EarlyStopping:
    ...
    def score_print(self, score):
        ...
        print(f'Value\t\t'+'\t'.join(format(eval, ".2f") for eval in eval10[:-1]))
        print('Metric\t\tHR@20\tMRR@20\tCov@20\tHRh@20\tMRRh@20\tHCov@20\tHRt@20\tMRRt@20\tTail@20\tTCov@20')
        print(f'Value\t\t' + '\t'.join(format(eval, ".2f") for eval in eval20[:-1]))
